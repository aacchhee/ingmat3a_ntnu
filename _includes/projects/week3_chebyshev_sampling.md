# Når målepunktene nesten mister et polynom
...
Vi skal undersøke polynomrommet $\mathcal P_n$ og transformasjonen som måler et
polynom i utvalgte punkter:
...
basis fast og endrer bare målepunktene. Ikke bland disse eksperimentene:
...
### Hva slags polynomer lager rekursjonen?
...
$(T_0,\ldots,T_n)$ ved høyere grad? Formuler en hypotese; du trenger ikke gi
et fullstendig induksjonsbevis.

## 2. Del 1 – måling som en lineær transformasjon

Vi går tilbake til lav grad før vi gjør store numeriske eksperimenter. La
$p\in\mathcal P_3$, og mål polynomet i punktene
...
sender altså et polynom til fire målte tall.
...
Dermed er første rad i målematrisen
...
1. Hva er inputrommet og outputrommet til $E$?
2. Hvorfor er `M_small` og `C_small` forskjellige når de beskriver samme
   transformasjon?
3. Hva forteller full rang om målingene?
4. Betyr forskjellig koordinatvektor at vi har fått et annet polynom?

### Rekonstruer det samme polynomet i to basiser

Vi lager måleverdier fra
...
print("Målinger:", measurements_small)
...
polynomer med identiske tre målinger:
...
print("Målinger av p:       ", first_values)
print("Målinger av p+alpha*z:", second_values)
print("Forskjell i målingene:", second_values-first_values)
...
plt.title("Forskjellige polynomer, identiske målinger")
plt.grid(alpha=0.25)
plt.legend()
plt.show()
```
...
retninger forventer du når vi måler et polynom i $\mathcal P_n$ i bare $m$
forskjellige punkter, der $m<n+1$? Begrunn svaret med rang–nullitet, og oppgi
forutsetningene du bruker.

## 3. Del 2 – samme målepunkter, forskjellig basis

Ved grad 3 virker begge basisene uproblematiske. Nå øker vi graden, men holder
målepunktene helt faste mens vi bytter basis. Da kan en eventuell forskjell
ikke skyldes at vi målte polynomet andre steder.
...
def chebyshev_points(n):
    """Returner n+1 målepunkter i intervallet [-1,1]."""
    k = np.arange(n+1)
...
1. Er måleverdiene identiske i de to rekonstruksjonene?
2. Hvor mange størrelsesordener skiller de største koordinatene?
3. Hvor på intervallet er rekonstruksjonsfeilen størst?
4. Har vi byttet polynomrom, transformasjon eller målepunkter?
...
plt.title("Samme polynom og samme målepunkter")
plt.grid(alpha=0.25)
plt.legend()
plt.show()
```
...
### En kontrollert forstyrrelse av målingene

Til slutt endrer vi hver måleverdi med omtrent $10^{-10}$, men beholder
...
print("Største endring i måling:", np.max(np.abs(measurement_noise)))
...
## 4. Del 3 – samme basis, forskjellige målepunkter

Nå holder vi Chebyshev-basis fast. Det eneste vi endrer, er hvor polynomet
måles. Dermed undersøker vi et annet spørsmål enn i del 2.
...
### Se på måledesignet før du regner
...
axes[0].set_title("Like langt mellom målepunktene")
axes[1].scatter(cheb_points, np.zeros_like(cheb_points), marker="|", s=180,
                color="#d98900")
axes[1].set_title("Cosinusfordelte målepunkter")
for ax in axes:
    ax.set_yticks([])
...
Hvor er punktavstanden størst og minst i hvert design? Hvilket design ville
du valgt dersom du var spesielt redd for at polynomet skulle gjøre noe
uventet nær $-1$ eller $1$? Skriv hypotesen før perturbasjonsforsøket.

### En liten, identisk målefeil

Vi forstyrrer målingene med det deterministiske mønsteret
...
for name, data in [("Jevne punkter", equal_data),
    ...
    print(f"{name:27s}",
          f"målefeil={data['max_data_error']:.3e}",
...
ikke skyldes ulik perturbasjon eller ulik basis.

### Det nesten usynlige polynomet

Forskjellen mellom den rekonstruerte og opprinnelige kurven er selv et
polynom:

$$r(x)=p_{\mathrm{forstyrret}}(x)-p_{\mathrm{opprinnelig}}(x).$$

Ved målepunktene er $r(x_k)$ bare den lille måleforstyrrelsen. Mellom punktene
vet vi ennå ikke hvor stort polynomet kan bli.
...
for ax, title, data, color in [
    ...
    ax.plot(data["grid"], data["error"], color=color,
            label="$r(x)$ mellom målingene")
    ax.scatter(data["points"], data["noise"], color="black", s=16,
               zorder=3, label="$r(x_k)$ ved målepunktene")
...
2. Hvor små er verdiene ved selve målepunktene?
3. Hvordan kan et polynom være nesten usynlig for målingene og samtidig stort
   mellom dem?
4. Hva minner dette om fra nullrommet i del 1?
5. Hvorfor er det ikke korrekt å kalle $r$ en eksakt nullromsvektor?
6. Hvordan henger feiltoppene sammen med fordelingen av målepunktene?

Et ikke-null polynom som måletransformasjonen sender til en svært liten
output, kan tolkes som en **numerisk nullromsretning**. Dette er en uformell
beskrivelse, ikke et nytt eksakt nullrom: Outputen er liten, men ikke null.
...
og om veksten ser jevn ut. Eksperimentet gir numerisk evidens; det er ikke et
bevis for hva som skjer for alle grader.
...
4. Er feilen her forårsaket av målepunktene?
...
Nå får du bruke ideene fra prosjektet mer fritt. Lag en målesituasjon som
forsterker en perturbasjon mest mulig, under disse reglene:

- polynomgraden skal være høyst 35;
- alle $n+1$ målepunkter skal være forskjellige og ligge i $[-1,1]$;
- hver målefeil skal ha absoluttverdi høyst $10^{-10}$;
- rekonstruksjonen skal gjøres i Chebyshev-basis;
- forsterkningen må måles på et tett rutenett med minst 5001 punkter.

Du kan endre graden, punktene og fortegnsmønsteret i perturbasjonen. Du kan
ikke gjøre målefeilen større eller legge to målepunkter nøyaktig oppå
hverandre.

```{pyodide-python}
# Startforslag. Bytt ut både punktene og perturbasjonen.
n_design = 20
...
plt.scatter(my_points, my_noise, color="black", s=15,
            label="feil ved målepunktene")
plt.xlabel("x")
plt.ylabel("feil")
plt.title("Min stresstest")
plt.grid(alpha=0.25)
plt.legend()
plt.show()
```
...
- Hva var nesten usynlig for måletransformasjonen?
...
4. begge målematrisene i lav grad og nullromseksperimentet;
5. tabell og feilfigur for basiseksperimentet;
6. tabell, punktfigur og feilpolynom for målepunktseksperimentet;
7. $T_{50}$-resultatene og forklaringen på sammenbruddet;
8. minst tre forsøk i den åpne stresstesten;
9. den samlede analysen.

Oppgi alltid grad, punkter, perturbasjonsstørrelse og rutenettstørrelse sammen
...
blande sammen effekten av basisvalg og målepunktvalg. Du må kunne forklare
...
Påstander om «stabilitet» skal støttes av konkrete perturbasjoner og målte
feil.
::::
